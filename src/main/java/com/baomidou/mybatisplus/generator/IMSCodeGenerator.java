...
public class IMSCodeGenerator {
    ...
    private static String[] includeArr = {"test_vss_workorder"};
    private static String tablePrefix = "test_vss_";
    private static String modelName = "imsVss";
...
//    }
    ...
    private static String parent = "com.ysten.conts.ecoms";

    public static void main(String[] args) {
        AutoGenerator autoGenerator = new AutoGenerator();
        /**选择模板引擎，默认 Veloctiy 可选 freemarker */
        ...
        strategy.setSuperServiceClass(parent+".service.base.IBaseService");
        strategy.setSuperServiceImplClass(parent+".service.base.impl.JpaBaseService");
        strategy.setSuperRepositoryClass(parent+".repository.base.IJpaBaseRepository");
        strategy.setSuperControllerClass(parent+".controller.base.BaseController");
...
//        tc.setXml("/template/mapper.xml.vm");
        ...
        fileOutList.add(new FileOutConfig("/template/elementall.vue.vm") {
            @Override
            public String outputFile(TableInfo tableInfo) {
                return outPutDir + "\\" +modelName + "\\" + "elementall.vue";
            }
        });
...
//        //设置过滤table的函数  TODO
//        autoGenerator.setDealTableFunction(tableInfo ->{
//            tableInfo.setPageDateQuery(true);   //有日期查询
//            tableInfo.setPageId("workorder_key");
//            tableInfo.setPageName("channl_workorder_no");
//            tableInfo.setPageUrl("imsVss/workorders");
//            List<TableField> fields = tableInfo.getFields();
//            fields.forEach(f -> {
//                //忽略字段
//               if(Arrays.asList("workorder_key vendor_key device_key partner_key produc_key".split(" ")).contains(f.getName())){
//                   f.setIgnore(true);
//               }
//                if(Arrays.asList("device_status industry_id operation_type order_status order_type screen_type".split(" ")).contains(f.getName())){
//                    f.setPageSearch(true);
//                }
//                if("filmorder_no".equals(f.getName())){      //唯一主键  新增的时候可编辑 修改的时候disabled
//                    f.setOnly(true);
//                }
////                if("area_id".equals(f.getName())){      //如果备注是是否上线:0|否,1|是 格式，自动设置为select
////                    f.setPageType("select");
////                }
//               if("description".equals(f.getName())){
//                   f.setPageType("text");
//               }
////                if("percentage".equals(f.getName())){
////                    f.setPageType("number");
////                }
////               else if("is_online".equals(f.getName())){
////                   f.setPageDefaultValue("1");
////               }
//            });
//            return tableInfo;
//        });
