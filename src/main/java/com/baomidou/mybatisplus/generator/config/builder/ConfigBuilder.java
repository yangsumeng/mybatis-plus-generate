...
public class ConfigBuilder {
  ...
  private void processTypes(StrategyConfig config) {
    ...
    if (StringUtils.isEmpty(config.getSuperRepositoryClass())) {
      superRepositoryClass = ConstVal.SUPERD_REPOSITORY_CLASS;
    } else {
  ...
  private List<TableInfo> processTable(
    ...
    for (TableInfo tableInfo : tableList) {
      ...
      if (StringUtils.isNotEmpty(globalConfig.getMapperName())) {
        tableInfo.setMapperName(
            String.format(globalConfig.getMapperName(), tableInfo.getEntityName()));
      } else {
  ...
  /**
   * 检查是否有 {@link com.baomidou.mybatisplus.annotations.TableId} {@link
   * com.baomidou.mybatisplus.annotations.TableField} 注解
   *
   * @param config
   * @param tableInfo
   * @param fieldPrefix
   */
  private void checkTableIdTableFieldAnnotation(
      StrategyConfig config, TableInfo tableInfo, String[] fieldPrefix) {
    ...
    if (importTableFieldAnnotaion) {
      tableInfo
          .getImportPackages()
          .add(com.baomidou.mybatisplus.annotations.TableField.class.getCanonicalName());
    }
    if (importTableIdAnnotaion) {
      tableInfo
          .getImportPackages()
          .add(com.baomidou.mybatisplus.annotations.TableId.class.getCanonicalName());
    }
    if (globalConfig.getIdType() != null) {
      ...
      tableInfo
          .getImportPackages()
          .add(com.baomidou.mybatisplus.enums.IdType.class.getCanonicalName());
    }
  }
  ...
  private TableInfo convertTableFields(TableInfo tableInfo, NamingStrategy strategy) {
    ...
    try {
      ...
      if (DbType.POSTGRE_SQL == dbQuery.dbType()) {
        tableFieldsSql =
            String.format(tableFieldsSql, dataSourceConfig.getSchemaname(), tableInfo.getName());
      } else {
